...
async def update_process_in_bigquery(
    prozess_id: str, update_data: Dict[str, Any]
) -> bool:
...
async def debug_prozess_search(prozess_id: str):
    ...
    result = {"prozess_id": prozess_id, "bigquery": None, "memory": None}
...
async def debug_warteschlange_data():
    ...
    try:
        ...
        result["total_status_updates"] = [dict(row) for row in bq_client.query(query2)][
            0
        ]
        ...
        """
        result["warteschlange_prozesse"] = [
            dict(row) for row in bq_client.query(query3)
        ][0]
        ...
        """
        result["gwa_warteschlange_berechtigt"] = [
            dict(row) for row in bq_client.query(query4)
        ][0]
...
async def get_warteschlangen_mit_status():
    ...
    if bq_client:
        try:
            ...
            return {"warteschlangen": queues, "source": "bigquery_status_updates"}
